#!/usr/bin/env python3
"""繪本／插畫風配樂：柔和鋼琴 + 弦樂墊 + 低音，純 Python 標準庫合成，零版權。

和 make_chiptune_bgm.py 是同一套思路（用 wave 直接寫取樣點），音色換成適合
插畫、繪本、慢節奏敘事的組合。chiptune 配 8-bit，這支配繪本／水彩／字卡。

改法：
  CHORDS   換和聲（每 4 秒一個）
  MELODY   換旋律（起, 音高 MIDI, 長度）
  secgain  換段落強弱——故事片的情緒轉折靠這個，不是靠換旋律
  DUR      對齊影片長度

內建的是一個「壓抑 → 打開」的 60 秒結構，可直接當骨架改：
  0–32s   Am / Dm 為主，低、窄、壓著
  28–36s  轉向 G（要有事情發生了）
  36–60s  C 大調打開，音域拉高，整體音量放大

用法：python3 make_bgm_storybook.py bgm.wav
"""
